...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("StartLifeMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("StartLifeMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("StartLifeMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("startlife-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running StartLifeMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
