...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop StartLife server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "StartLife server stopping";
}
...
static const CRPCCommand vRPCCommands[] =
    {
        ...
        /* StartLife features */
        {"startlife", "masternode", &masternode, true, true, false},
        {"startlife", "masternodelist", &masternodelist, true, true, false},
        {"startlife", "mnbudget", &mnbudget, true, true, false},
        {"startlife", "mnbudgetvoteraw", &mnbudgetvoteraw, true, true, false},
        {"startlife", "mnfinalbudget", &mnfinalbudget, true, true, false},
        {"startlife", "mnsync", &mnsync, true, true, false},
        {"startlife", "spork", &spork, true, true, false},
#ifdef ENABLE_WALLET
        {"startlife", "obfuscation", &obfuscation, false, false, true}, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
    ...
    if (((mapArgs["-rpcpassword"] == "") ||
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
                                             _("To use startlifed, or the -server option to startlife-qt, you must set an rpcpassword in the configuration file:\n"
                                               "%s\n"
...
{
    return "> startlife-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(string methodname, string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:27019/\n";
}
